...
"""PyInstaller spec for the bundled `humanai-server` (desktop sidecar).
...
archive to a temp dir on EVERY launch — 6-7s of "Starting Human AI…" splash (measured; the
actual Python import is ~0.5s). The wrinkles handled here:
  - aisuite is a regular pip dependency (git-pinned in pyproject.toml); collect Human AI +
...
# Onedir: dist/humanai-server/{humanai-server[.exe], _internal/}. The build scripts stage
